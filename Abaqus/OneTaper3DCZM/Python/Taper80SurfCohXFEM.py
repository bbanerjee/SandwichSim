...
if mdb.models.has_key('Taper80SurfCohXFEM'):
  del mdb.models['Taper80SurfCohXFEM']

taperModel = mdb.Model(name='Taper80SurfCohXFEM')
...
for ii in range(len(taperAssem.instances)):
  ...
  for jj in range(len(faceList)):
    ...
    #print zCentroid
    if zCentroid == 0.0 or zCentroid == sandwichThick:
      #print curFace
...
# Seed the part
topFacePart.seedPart(size=face2Thick/3, deviationFactor=0.1)
botFacePart.seedPart(size=face1Thick/3, deviationFactor=0.1)
...
taperCohContact.CohesiveBehavior(repeatedContacts=ON, defaultPenalties=OFF, 
  eligibility=SPECIFIED, table=((faceFaceEKnn, faceFaceG1Kss, faceFaceG2Ktt), ))
...
#### Second edge
e1 = boundaryUtils.getNextEdge3D(topEdgeList, topVertList, e1)
faces = e1.getFaces()
topFace = topFaceList[0:0]
for jj in range(len(faces)):
  curFace = topFaceList[faces[jj]]
...
#### Second edge
e1c = boundaryUtils.getNextEdge3D(botEdgeList, botVertList, e1c)
faces = e1c.getFaces()
...
mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF,
  ...
  memory=90, memoryUnits=PERCENTAGE, model='Taper80SurfCohXFEM', modelPrint=OFF,
  multiprocessingMode=DEFAULT, name='Taper80SurfCohXFEM', nodalOutputPrecision=SINGLE,
