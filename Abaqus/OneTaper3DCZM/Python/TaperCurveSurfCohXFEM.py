...
if mdb.models.has_key('TaperCurveSurfCohXFEM'):
  del mdb.models['TaperCurveSurfCohXFEM']

taperModel = mdb.Model(name='TaperCurveSurfCohXFEM')
...
edgeList = taperPart.edges
pEdgeMid = (0.5*(x11+x2), 0.0, 0.0)
edgeToPartition = edgeList.findAt(pEdgeMid)
tPartition = 1.0 - (x2-x11-face1Thick)/(x2-x11)
taperPart.PartitionEdgeByParam(edges=edgeToPartition, parameter=tPartition)

pLoc = (x11+face1Thick, 0, 0)
cellList = taperPart.cells
pCell1 = (0.5*(x11+x3), 0, 0)
pCell2 = (0.5*(x11+x3), face1Thick-0.5*cohesiveThick, 0)
pCell3 = (x10a-cohesiveThick, 0.5*(y11a+y10a), 0)
cellToPartition = cellList.findAt((pCell1,), (pCell2,), (pCell3,))
taperPart.PartitionCellByPlaneNormalToEdge(edge=edgeToPartition, point=pLoc,
...
pCellMid6 = (0.5*(x11+face1Thick+x5), 0.5*(y5), midPanel)
cellList = taperPart.cells
...
#----------------------------------------------------------------
# Delete the cohesive cells
#----------------------------------------------------------------
#removeCellList = cellList[0:0]
#removeCellList += setCohesiveFaceFace.cells
#removeCellList += setCohesiveFaceCoreBot.cells
#removeCellList += setCohesiveFaceCoreTaper.cells
#removeCellList += setCohesiveFaceCoreTop.cells
#removeCellList += setCohesiveFaceCoreCurve1.cells
#removeCellList += setCohesiveFaceCoreCurve2.cells
#taperPart.RemoveCells(removeCellList)
...
# Sweep path for the cohesive elements
# edgeList = taperPart.edges
# sweepEdge = edgeList.findAt((x11, y11-face2Thick-0.5*cohesiveThick))
# taperPart.setSweepPath(region=setCohesiveFaceCoreCurve1.cells, edge=sweepEdge,
#   sense=FORWARD)
# sweepEdge = edgeList.findAt((x10, y10-face2Thick-0.5*cohesiveThick))
# taperPart.setSweepPath(region=setCohesiveFaceCoreCurve2.cells, edge=sweepEdge,
#   sense=FORWARD)
...
topFaceList = faceList[0:0]
botFaceList = faceList[0:0]
cellList = taperInst.cells
faceList = taperInst.faces
vertList = taperInst.vertices
edgeList = taperInst.edges
...
e1 = edgeList.findAt(pp)
faces = e1.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
  center = curFace.getCentroid()
  topFace = faceList.findAt(center)
  zCenter = center[0][2]
...
e1c = edgeList.findAt(ppc)
faces = e1c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
  center = curFace.getCentroid()
  botFace = faceList.findAt(center)
  zCenter = center[0][2]
  if (zCenter < sandwichThick):
    break
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intTop2 = taperModel.SurfaceToSurfaceContactStd(name='Int-2',
...
### Second edge
e2 = boundaryUtils.getNextEdge3D(edgeList, vertList, e1)
faces = e2.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#### Bottom face
e2c = boundaryUtils.getNextEdge3D(edgeList, vertList, e1c)
faces = e2c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
e2c = boundaryUtils.getNextEdge3D(edgeList, vertList, e2c)
faces = e2c.getFaces()
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
  #center = curFace.getCentroid()
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intTop3 = taperModel.SurfaceToSurfaceContactStd(name='Int-3',
#  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
#  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
#  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)

### Third edge
e3 = boundaryUtils.getNextEdge3D(edgeList, vertList, e2)
faces = e3.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#### Bottom face
e3c = boundaryUtils.getNextEdge3D(edgeList, vertList, e2c)
faces = e3c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intTop4 = taperModel.SurfaceToSurfaceContactStd(name='Int-4',
#  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
#  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
#  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)

### Fourth edge
e4 = boundaryUtils.getNextEdge3D(edgeList, vertList, e3)
faces = e4.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#### Bottom face
e4c = boundaryUtils.getNextEdge3D(edgeList, vertList, e3c)
faces = e4c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intTop5 = taperModel.SurfaceToSurfaceContactStd(name='Int-5',
#  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
#  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
#  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)

### Fifth edge
e5 = boundaryUtils.getNextEdge3D(edgeList, vertList, e4)
faces = e5.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#### Bottom face
e5c = boundaryUtils.getNextEdge3D(edgeList, vertList, e4c)
faces = e5c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intTop6 = taperModel.SurfaceToSurfaceContactStd(name='Int-6',
#  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
#  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
#  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)

### Sixth edge
e6 = boundaryUtils.getNextEdge3D(edgeList, vertList, e5)
faces = e6.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
#### Bottom face
e6c = boundaryUtils.getNextEdge3D(edgeList, vertList, e5c)
faces = e6c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
  curFace = faceList[faces[jj]]
...
region1 = regionToolset.Region(side1Faces=topFaceList)
region2 = regionToolset.Region(side2Faces=botFaceList)
intTop7 = taperModel.SurfaceToSurfaceContactStd(name='Int-7',
  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)
...
### First edge
#### Top face
pp = (x10-cohesiveThick, face1Thick, sandwichThick)
e1 = edgeList.findAt(pp)
faces = e1.getFaces()
topFace = faceList[0:0]
for jj in range(len(faces)):
...
#### Bottom face
ppc = (x10-cohesiveThick, face1Thick-cohesiveThick, sandwichThick)
e1c = edgeList.findAt(ppc)
faces = e1c.getFaces()
botFace = faceList[0:0]
for jj in range(len(faces)):
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intBot2 = taperModel.SurfaceToSurfaceContactStd(name='Int-Bot-2',
#  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
#  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
#  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)
...
#region1 = regionToolset.Region(side1Faces=topFace)
#region2 = regionToolset.Region(side2Faces=botFace)
#intBot3 = taperModel.SurfaceToSurfaceContactStd(name='Int-Bot-3',
#  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
#  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
#  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)
...
region1 = regionToolset.Region(side1Faces=topFaceList)
region2 = regionToolset.Region(side2Faces=botFaceList)
intBot4 = taperModel.SurfaceToSurfaceContactStd(name='Int-Bot-4',
  createStepName='Initial', master=region1, slave=region2, sliding=FINITE,
  thickness=ON, interactionProperty='IntProp-2', adjustMethod=NONE,
  initialClearance=OMIT, datumAxis=None, clearanceRegion=None)

# Change to node to surface contact
#intTop2.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
#intTop3.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
#intTop4.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
#intTop5.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
#intTop6.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
intTop7.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
#intBot2.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
#intBot3.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
#  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
#  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
intBot4.setValues(initialClearance=OMIT, surfaceSmoothing=NONE, adjustMethod=NONE,
  sliding=FINITE, enforcement=NODE_TO_SURFACE, thickness=OFF,
  supplementaryContact=SELECTIVE, smooth=0.2, bondingSet=None)
...
mdb.Job(atTime=None, contactPrint=OFF, description='', echoPrint=OFF,
  ...
  memory=90, memoryUnits=PERCENTAGE, model='TaperCurveSurfCohXFEM', modelPrint=OFF,
  multiprocessingMode=DEFAULT, name='TaperCurveSurfCohXFEM', nodalOutputPrecision=SINGLE,
